...
locals {
  ...
  cicd_workflows = {
    for k, v in local.cicd_repositories : k => templatefile(
      "${path.module}/templates/workflow-${v.type}.yaml", {
        ...
        build_log_bucket = module.build-log-gcs.name
      }
    )
  }
  custom_roles = {
  ...
  tfvars = {
    automation = {
      ...
      module_repository = try(module.automation-tf-modules-repo[0].name, "SOURCEREPO_CICD_MODULE_REPOSITORY_NAME_DISABLED")
    }
...
# Slalom: Cloud Storage bucket to store build logs
output "build_log_bucket" {
  description = "GCS bucket where Cloud Build stores build logs."
  value       = module.build-log-gcs.name
}

# Slalom: Separate Cloud Source repository for Terraform modules
#         Please note: Google Cloud Source Repositories isn't available to new customers.
output "cicd_module_repository" {
  description = "(Deprecated) CI/CD Terraform modules Google Cloud Source repository."
  value = {
    id   = try(module.automation-tf-modules-repo[0].id, "SOURCEREPO_CICD_MODULE_REPOSITORY_ID_DISABLED")
    name = try(module.automation-tf-modules-repo[0].name, "SOURCEREPO_CICD_MODULE_REPOSITORY_NAME_DISABLED")
    url  = try(module.automation-tf-modules-repo[0].url, "SOURCEREPO_CICD_MODULE_REPOSITORY_URL_DISABLED")
  }
}
