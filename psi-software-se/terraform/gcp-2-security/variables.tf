...
variable "postfix" {
  description = "Postfix (suffix) used for resources that need unique names. Use only one character."
  type        = string
  default     = "0"
  validation {
    condition     = try(length(var.postfix), 0) < 2
    error_message = "Use only one character for postfix."
  }
}
