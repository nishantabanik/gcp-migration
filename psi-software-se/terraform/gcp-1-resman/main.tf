...
locals {
  ...
  branch_optional_sa_lists = {
    ...
    pf-sbox   = compact([try(module.branch-pf-sbox-sa[0].iam_email, "")])
    pf-legacy = compact([try(module.branch-pf-legacy-sa[0].iam_email, "")])
    pf-c2p    = compact([try(module.branch-teams-team-sa["c2p"].iam_email, "")])
  }
  cicd_repositories = {
