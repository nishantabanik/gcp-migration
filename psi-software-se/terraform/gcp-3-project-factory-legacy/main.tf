/**
 * Copyright 2023-2025 Slalom GmbH
...
locals {
  ...
  _defaults_net = {
    ...
    shared_vpc_self_link = try(var.vpc_self_links["legacy-spoke-0"], null)
    vpc_host_project     = try(var.host_project_ids["legacy-spoke-0"], null)
    # Slalom: DNS zone suffix for VPC spoke (environment).
    environment_dns_zone = try(var.vpc_dns_domains["legacy-spoke-0"], null)
  }
...
module "projects" {
  ...
  prefix                 = var.prefix
  service_accounts       = merge(try(each.value.service_accounts, {}), try(local._sa[each.key], {}))
  service_accounts_iam   = merge(try(each.value.service_accounts_iam, {}), try(local._sa_iam[each.key], {}))
