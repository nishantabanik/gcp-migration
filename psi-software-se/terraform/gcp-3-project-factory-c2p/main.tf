...
locals {
  ...
  _defaults_net = {
    ...
    shared_vpc_self_link = try(var.vpc_self_links["c2p-spoke-0"], null)
    vpc_host_project     = try(var.host_project_ids["c2p-spoke-0"], null)
    # Slalom: DNS zone suffix for VPC spoke (environment).
    environment_dns_zone = try(var.vpc_dns_domains["c2p-spoke-0"], null)
  }
