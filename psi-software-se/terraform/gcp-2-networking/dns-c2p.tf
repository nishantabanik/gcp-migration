...
module "c2p-dns-priv" {
  ...
  name        = var.dns_zone_names.c2p
  domain      = var.dns_zone_domains.c2p
  description = "Private zone for ${var.dns_zone_domains.c2p} (TF managed)"
  # Slalom: Create private zone in shared VPC spoke
  #client_networks = [module.landing-vpc.self_link]
  client_networks = [module.c2p-spoke-vpc.self_link]
  recordsets = {
...
module "landing-root-dns-peer-c2p" {
  source          = "git@gitlab.com:psi-software-se/terraform/modules.git//dns?ref=stable"
  project_id      = module.landing-project.project_id
  type            = "peering"
  name            = var.dns_zone_names.c2p
  domain          = var.dns_zone_domains.c2p
  description     = "Peering zone for ${var.dns_zone_domains.c2p} (TF managed)"
  client_networks = [module.landing-vpc.self_link]
  peer_network    = module.c2p-spoke-vpc.self_link
}


module "c2p-dns-peer-landing-root" {
  source          = "git@gitlab.com:psi-software-se/terraform/modules.git//dns?ref=stable"
  project_id      = module.c2p-spoke-project.project_id
  type            = "peering"
  name            = "c2p-root-dns-peering"
  domain          = "."
...
module "c2p-dns-peer-landing-rev-10" {
  source          = "git@gitlab.com:psi-software-se/terraform/modules.git//dns?ref=stable"
  project_id      = module.c2p-spoke-project.project_id
  type            = "peering"
  name            = "c2p-reverse-10-dns-peering"
