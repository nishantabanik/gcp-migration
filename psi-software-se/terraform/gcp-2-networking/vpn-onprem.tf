...
# Slalom: CFIT-628 add static VPN gateway
module "landing-to-onprem-static-primary-vpn" {
  count         = var.vpn_onprem_static_primary_config == null ? 0 : 1
  source        = "git@gitlab.com:psi-software-se/terraform/modules.git//net-vpn-static?ref=stable"
  project_id    = module.landing-project.project_id
  network       = module.landing-vpc.self_link
  region        = var.regions.primary
  name          = "vpn-static-to-onprem-${local.region_shortnames[var.regions.primary]}"
  remote_ranges = var.vpn_onprem_static_primary_config.remote_ranges
}
