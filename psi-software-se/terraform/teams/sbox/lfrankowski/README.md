...
docker pull nginx
docker tag nginx:latest europe-west3-docker.pkg.dev/psi-de-0-sbox-lfrankowski/container-images/nginx:latest
docker push europe-west3-docker.pkg.dev/psi-de-0-sbox-lfrankowski/container-images/nginx:latest
```
...
* add GKE cluster to local `.kube/config`
    ...
    # example:
    gcloud container clusters get-credentials gke-sbox-lfrankowski-1 --region europe-west3 --project psi-de-0-sbox-lfrankowski
    ```
...
* verify by making example `kubectl` calls
    ...
    # start nginx:
    kubectl run nginx --image=europe-west3-docker.pkg.dev/psi-de-0-sbox-lfrankowski/container-images/nginx --port=80
    # forward port:
