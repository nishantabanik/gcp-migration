# dev: Infrastructure as a Code

[![Open in Dev Container](https://img.shields.io/static/v1?label=Dev%20Containers&message=Open%20in%20Dev%20Container&color=green)](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=git@gitlab.com:psi-software-se/leg-ibm/iac/teams/gem/non-prod-ext/dev.git)
...
gcloud config set project psi-de-0-ibm-gem-dev-iac
```
...
docker pull nginx
docker tag nginx:latest europe-west3-docker.pkg.dev/psi-de-0-ibm-gem-dev-iac/container-images/nginx:latest
docker push europe-west3-docker.pkg.dev/psi-de-0-ibm-gem-dev-iac/container-images/nginx:latest
```
...
* add GKE cluster to local `.kube/config`
    ...
    # example:
    gcloud container clusters get-credentials gke-dev-1 --region europe-west3 --project psi-de-0-ibm-gem-dev-iac
    ```
...
* verify by making example `kubectl` calls
    ...
    # start nginx:
    kubectl run nginx --image=europe-west3-docker.pkg.dev/psi-de-0-ibm-gem-dev-iac/container-images/nginx --port=80
    # forward port:
