# ibm-iai-elmar-rnd: Infrastructure as a Code

[![Open in Dev Container](https://img.shields.io/static/v1?label=Dev%20Containers&message=Open%20in%20Dev%20Container&color=green)](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=git@gitlab.com:psi-software-se/terraform/teams/ibm/cf-iai/prod-ext/elmar.git)
...
gcloud config set project psi-de-0-ibm-iai-elmar-rnd
```
...
docker pull nginx
docker tag nginx:latest europe-west3-docker.pkg.dev/psi-de-0-ibm-iai-elmar-rnd/container-images/nginx:latest
docker push europe-west3-docker.pkg.dev/psi-de-0-ibm-iai-elmar-rnd/container-images/nginx:latest
```
...
* add GKE cluster to local `.kube/config`
    ...
    # example:
    gcloud container clusters get-credentials gke-1 --region europe-west3 --project psi-de-0-ibm-iai-elmar-rnd
    ```
...
* verify by making example `kubectl` calls
    ...
    # start nginx:
    kubectl run nginx --image=europe-west3-docker.pkg.dev/psi-de-0-ibm-iai-elmar-rnd/container-images/nginx --port=80
    # forward port:
